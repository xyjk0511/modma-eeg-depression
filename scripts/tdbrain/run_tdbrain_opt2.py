"""TDBRAIN optimized CV v2: Hjorth features + nested threshold + grid search."""
...
def compute_hjorth(X, ch_names):
    """Hjorth activity/mobility/complexity per region. Returns (n_win, 12)."""
...
def optimal_threshold(y_true, y_prob):
    ...
    ba = (tpr + (1 - fpr)) / 2
    return thresholds[np.argmax(ba)]


def subject_cv_nested_threshold(features, y, groups, n_splits, seed, C_val):
    """CV with nested threshold: train threshold on inner CV, evaluate on outer."""
    ...
    # Nested threshold via LOO on subject predictions
    n_subj = len(yt)
    preds_nested = np.zeros(n_subj)
    for i in range(n_subj):
        mask = np.ones(n_subj, dtype=bool)
        mask[i] = False
        thr = optimal_threshold(yt[mask], yp[mask])
        preds_nested[i] = 1 if yp[i] >= thr else 0
    ba_nested = balanced_accuracy_score(yt, preds_nested)

    return {"ba": round(ba_fixed, 4), "ba_nested": round(ba_nested, 4),
...
def main():
    ...
    logger.info("Loading TDBRAIN (formal_status) with raw windows...")
    ...
    feat_extended = np.column_stack([features, hjorth_feat])
    logger.info(f"Extended features: {feat_extended.shape[1]} dims (15 base + {hjorth_feat.shape[1]} Hjorth)")
    ...
    feat_sets = {
        "5dim": features[:, :5],
        "15dim": features,
        "15dim+hjorth": feat_extended,
        "5dim+hjorth": np.column_stack([features[:, :5], hjorth_feat]),
    }

    results = []
    for C in [0.1, 1.0, 10.0]:
        for fname, fdata in feat_sets.items():
            try:
                r = subject_cv_nested_threshold(fdata, y, groups, n_splits, args.seed, C)
                entry = {"C": C, "features": fname, "n_feat": fdata.shape[1],
                         "ba": r["ba"], "ba_nested": r["ba_nested"], "auc": r["auc"]}
                results.append(entry)
                logger.info(f"C={C:<5} {fname:>15s} ({fdata.shape[1]:2d}d): "
                           f"BA={r['ba']:.3f} BA_nested={r['ba_nested']:.3f} AUC={r['auc']:.3f}")
            except Exception as e:
                logger.warning(f"C={C} {fname}: FAILED - {e}")

    best = max(results, key=lambda x: (x["ba_nested"], x["auc"]))
    logger.info(f"\nBest: C={best['C']} {best['features']}: "
                f"BA={best['ba']:.3f} BA_nested={best['ba_nested']:.3f} AUC={best['auc']:.3f}")

    # Permutation test on best
    best_feat = feat_sets[best["features"]]
    ...
    for i in range(args.n_permutations):
        ...
        try:
            r = subject_cv_nested_threshold(best_feat, y_perm, groups, n_splits, args.seed, best["C"])
    ...
    output = {
        ...
        "hjorth_names": hjorth_names, "elapsed_seconds": elapsed,
    }
