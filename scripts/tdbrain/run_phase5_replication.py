"""Phase 5: Confirmatory replication — train MODMA, test TDBRAIN.

Train locked pipeline on full MODMA, test on external TDBRAIN dataset,
run cross-dataset permutation test, save complete report.
"""
import argparse
import json
import logging
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="Phase 5: Train MODMA, test TDBRAIN replication"
    )
    p.add_argument("--bids-root", required=True, help="Path to MODMA BIDS dataset")
    p.add_argument("--tdbrain-root", required=True, help="Path to TDBRAIN dataset root")
    p.add_argument("--output-dir", default=str(ROOT / "outputs/results_phase5"), help="Output directory")
...
def run_replication(args):
    """Main replication: train MODMA, test TDBRAIN, permutation test, report."""
    os.makedirs(args.output_dir, exist_ok=True)
    t0 = time.time()

    # --- 1. Load and prepare MODMA training data ---
    ...
    # Per-window QC (200uV, 12% dynamic bad)
    n_ch = X_win.shape[1]
    ...
    # --- 2. Train on full MODMA (no CV split) ---
    logger.info("Training pipeline on full MODMA...")
    ...
    # --- 3. Load external TDBRAIN test data ---
    ...
    # --- 4. Predict on external (MODMA-fitted pipeline) ---
    ...
    # --- 6. Build report ---
    report = {
        "experiment_type": "cross_dataset_replication_v1",
    ...
    # Conclusion using existing function
    ...
    ax.set_xlabel("FPR")
    ax.set_ylabel("TPR")
    ax.set_title("Cross-Dataset ROC (Train:MODMA, Test:TDBRAIN)")
    ax.legend()
