...
gaps:
  - truth: "hcue p-value printed to stdout"
    status: partial
    reason: "Print statement and wiring exist; SUMMARY claims p=0.022 but no captured log corroborates the numeric result"
    artifacts:
      - path: "run_modma_erp.py"
        issue: "No stdout log or results file captures the actual run output"
    missing:
      - "Captured stdout confirming p=0.022 from an actual execution"
human_verification:
  - test: "cd D:/eeg && python run_modma_erp.py 2>&1 | grep 'Observed BA'"
    expected: "Observed BA=0.670  p=0.0220  (perm mean=~0.494)"
...
| 3 | hcue p-value printed to stdout | PARTIAL | Print at line 144 wired in `__main__` at line 196; claimed p=0.022 unconfirmed by log |
...
| `08-01-SUMMARY.md` | BA and p-value recorded | VERIFIED | BA=0.670, p=0.0220, perm mean=0.494, N=52 |
...
| ERP-09 | Validate on hcue single condition | SATISFIED (code) / NEEDS HUMAN (result) | `__main__` hcue block wired; p=0.022 claimed in SUMMARY |
...
Implementation is complete and correctly wired. The single gap is evidentiary: p=0.022 is recorded only in SUMMARY.md with no captured stdout log. This is a documentation gap, not an implementation gap. Re-running the script with fixed seed=42 should reproduce the result deterministically.
